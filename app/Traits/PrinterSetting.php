...
trait PrinterSetting
{
    ...
    private function printKotOrderInfo($order, $kot)
    {
        ...
        // Print order name if it exists
        if ($order->name) {
            $this->printer->setEmphasis(true);
            $this->printer->text(__('modules.order.orderName') . ': ' . $order->name . "\n");
            $this->printer->setEmphasis(false);
        }
    ...
    private function printOrderDetails($order, $receiptSettings)
    {
        ...
        // Print order name if it exists
        if ($order->name) {
            $this->printer->text($leftPad . str_pad(__('modules.order.orderName'), $qtyWidth, ' ', STR_PAD_LEFT) . $order->name . "\n");
        }
    ...
    private function printOrderFooter($receiptSettings, $order, $printerSetting)
    {
        ...
        $this->printThankYouMessage($receiptSettings);
    ...
    private function printThankYouMessage($receiptSettings = null)
    {
        $this->printer->setJustification(Printer::JUSTIFY_CENTER);
        $footerMessage = $receiptSettings && $receiptSettings->footer_message 
            ? $receiptSettings->footer_message 
            : __('messages.thankYouVisit');
        $this->printer->text($footerMessage . "\n");
    }
