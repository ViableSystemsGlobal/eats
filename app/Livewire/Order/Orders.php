...
class Orders extends Component
{
    ...
    public function mount()
    {
        ...
        if (request()->has('status')) {
            $this->filterOrders = request()->get('status');
        }
    ...
    public function render()
    {
        ...
        // Create unserved orders filter (same logic as widget)
        $unservedOrders = $orders->filter(function ($order) {
            return !in_array($order->order_status->value, ['served', 'delivered', 'cancelled']);
        });
        ...
        return view('livewire.order.orders', [
            ...
            'unservedOrdersCount' => count($unservedOrders),
