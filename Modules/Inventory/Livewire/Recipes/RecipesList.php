        ...
        $query = Recipe::query()
            ...
            ->when($this->search, function ($query) {
                $query->where(function($q) {
                    $q->where('menu_items.item_name', 'like', '%' . $this->search . '%')
                      ->orWhereExists(function($subquery) {
                          $subquery->select(DB::raw(1))
                                  ->from('recipes')
                                  ->join('inventory_items', 'recipes.inventory_item_id', '=', 'inventory_items.id')
                                  ->whereColumn('recipes.menu_item_id', 'menu_items.id')
                                  ->where('inventory_items.name', 'like', '%' . $this->search . '%');
                      });
                });
            })
            ->when($this->category, function ($query) {
                $query->where('item_categories.category_name', 'like', '%' . $this->category . '%');
            })
