<div
    ...
    <div>
        ...
        <div class="flex justify-between my-2 items-center">
            <div class="font-medium py-2 inline-flex items-center gap-2  dark:text-neutral-200">
                ...
                <span>@lang('modules.order.orderNumber') #{{ $orderNumber }}</span>
                @if($orderName)
                    <span class="text-sm font-normal text-gray-600 dark:text-gray-400">- {{ $orderName }}</span>
                @endif
            </div>

            {{-- Customer Display Button --}}
            <div class="inline-flex items-center gap-2">
                <x-secondary-button onclick="window.open('{{ route('pos.customer-display') }}', 'customer-display', 'width=800,height=600,scrollbars=yes,resizable=yes')" 
                    class="inline-flex items-center gap-1 text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-display" viewBox="0 0 16 16">
                        <path d="M0 4s0-2 2-2h12s2 0 2 2v6s0 2-2 2H2s-2 0-2-2V4zm1.398-.855a.758.758 0 0 0-.254.302A1.46 1.46 0 0 0 0 4.01V10c0 .325.078.502.145.602.07.105.157.175.253.303a.51.51 0 0 0 .544.425h4.701a.5.5 0 0 0 .544-.425c.096-.128.182-.198.253-.303A1.46 1.46 0 0 0 7 10V4.01A1.46 1.46 0 0 0 6.854.145a.758.758 0 0 0-.254-.302A.51.51 0 0 0 6.05 0h-4.7a.51.51 0 0 0-.544.425z"/>
                    </svg>
                    Customer Display
                </x-secondary-button>
            </div>
        ...
        <div class="flex justify-between items-center gap-2">
            ...
            {{-- Order Name Button - Available for all order types (dine_in, pickup, delivery) --}}
            <div class="gap-2 inline-flex items-center ml-auto">
                <x-secondary-button class="relative" wire:click="$toggle('showOrderNameModal')" :title="__('modules.order.addOrderName')" data-tooltip-target="tooltip-order-name">
                    @if ($this->orderName)
                    <svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" fill="currentColor" class="absolute bi bi-circle-fill top-1 right-1 text-skin-base" viewBox="0 0 16 16"><circle cx="8" cy="8" r="8"/></svg>
                    @endif
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-tag" viewBox="0 0 16 16">
                        <path d="M6 4.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m-1 0a.5.5 0 1 0-1 0 .5.5 0 0 0 1 0"/>
                        <path d="M2 2h4.586a1 1 0 0 1 .707.293l7 7a1 1 0 0 1 0 1.414l-4.586 4.586a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 2 6.586zm4.586-.707a2 2 0 0 0-2.828 0L1 5.172a2 2 0 0 0 0 2.828l7 7a2 2 0 0 0 2.828 0L15 9.828a2 2 0 0 0 0-2.828z"/>
                    </svg>
                </x-secondary-button>

                <div id="tooltip-order-name" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                    @lang('modules.order.addOrderName')
                    <div class="tooltip-arrow" data-popper-arrow></div>
                </div>
            </div>
    ...
    <div>
        <div class="h-auto p-4 mt-3 select-none text-center w-full bg-gray-50 rounded space-y-4 dark:bg-gray-700">
            ...
            @php
                // Use the new tax calculation service with format setting
                $taxFormat = restaurant()->receiptSetting->tax_format ?? 'current';
                $taxCalculation = \App\Services\TaxCalculationService::getTaxCalculation(
                    $subTotal, 
                    $discountAmount ?? 0, 
                    restaurant()->id,
                    $taxFormat
            ...
            @if ($taxFormat === 'reverse')
                {{-- Reverse Format: Total is the base food price, broken down into components --}}
                <div class="flex justify-between text-sm font-semibold text-gray-700 dark:text-neutral-200">
                    <div>@lang('modules.order.total') (Base Food Price)</div>
                    <div>{{ currency_format($taxCalculation['baseAmount'], restaurant()->currency_id) }}</div>
                </div>

                {{-- Breakdown of components --}}
                <div class="flex justify-between text-sm text-gray-500 dark:text-neutral-400">
                    <div>@lang('modules.order.foodCost')</div>
                    <div>{{ currency_format($taxCalculation['foodCost'], restaurant()->currency_id) }}</div>
                </div>
                ...
                @endif
            @else
                {{-- Current Format: Traditional tax calculation --}}
                {{-- Display Levies First --}}
                @foreach ($taxCalculation['levies'] as $levy)
                <div class="flex justify-between text-gray-500 text-sm dark:text-neutral-400">
                    <div>{{ $levy['tax']->tax_name }} ({{ $levy['tax']->tax_percent }}%) <span class="text-gray-400 text-xs">(Levy)</span></div>
                    <div>{{ currency_format($levy['amount'], restaurant()->currency_id) }}</div>
                </div>
                @endforeach

                {{-- Show intermediate subtotal after levies --}}
                <div class="flex justify-between text-sm font-semibold text-gray-700 dark:text-neutral-200">
                    <div>@lang('modules.order.subtotalAfterLevies')</div>
                    <div>{{ currency_format($taxCalculation['subtotalAfterLevies'], restaurant()->currency_id) }}</div>
                </div>

                {{-- Display VAT on subtotal after levies --}}
                @if ($taxCalculation['vat'])
                <div class="flex justify-between text-gray-500 text-sm dark:text-neutral-400">
                    <div>{{ $taxCalculation['vat']['tax']->tax_name }} ({{ $taxCalculation['vat']['tax']->tax_percent }}%) <span class="text-gray-400 text-xs">(VAT)</span></div>
                    <div>{{ currency_format($taxCalculation['vat']['amount'], restaurant()->currency_id) }}</div>
                </div>
                @endif
            @endif
