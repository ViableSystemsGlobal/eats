<div class="flex flex-col h-auto min-h-screen px-2 py-4 pr-4 bg-white border-l lg:w-6/12 dark:border-gray-700 dark:bg-gray-800">
    <div>
        ...
        <div class="flex items-center justify-between mt-1 mb-2">
            <div class="inline-flex items-center gap-2 py-2 font-medium dark:text-neutral-200">
                ...
                @if($orderDetail && $orderDetail->modification_count > 0)
                    <span class="px-2 py-1 text-xs font-semibold text-orange-600 bg-orange-100 rounded-full dark:bg-orange-900 dark:text-orange-300">
                        @lang('modules.order.orderModified')
                    </span>
                @endif
            </div>
        ...
        {{-- Show new items in cart (not yet in any KOT) --}}
        @php
            $newItems = [];
            $kotItemKeys = [];
            ...
            foreach ($orderItemList as $key => $item) {
                if (!in_array($key, $kotItemKeys) && strpos($key, 'kot_') === false) {
                    $newItems[$key] = $item;
                }
            }
        @endphp
        ...
        @if (!empty($newItems))
            <div class="flex justify-between p-2 text-xs font-medium text-gray-500 bg-blue-100 dark:bg-blue-900">
                <div>@lang('modules.order.newItems')</div>
            </div>

            <div class="flex flex-col rounded mb-4">
            ...
            <div class="flex flex-col rounded">
                <table class="flex-1 min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='menu-item-list-{{ microtime() }}'>
                        ...
                        @continue(strpos($key, 'kot_' . $kot->id) === false)
        ...
        <div class="w-full h-auto pt-3 pb-4 select-none">
            @if ($orderDetail->status == 'kot' && user_can('Update Order'))
                <div class="grid grid-cols-2 gap-4">
                    {{-- KOT button for creating incremental KOTs for new items --}}
                    <button class="w-full p-2 text-white rounded bg-gray-700" wire:click="saveOrder('kot')" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="saveOrder('kot')">@lang('modules.order.kot')</span>
                        <span wire:loading wire:target="saveOrder('kot')" class="inline-flex items-center justify-center">
