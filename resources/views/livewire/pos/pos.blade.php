<div>
    ...
    <x-dialog-modal wire:model.live="showOrderNameModal" maxWidth="xl">
        <x-slot name="title">
        ...
        <x-slot name="content">
            <div>
                <x-label for="orderName" :value="__('modules.order.orderName')" />
                <x-input id="orderName" type="text" class="block mt-1 w-full" wire:model='orderName' 
                    placeholder="{{ __('modules.order.orderNamePlaceholder') }}" />
                <x-input-error for="orderName" class="mt-2" />
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-button wire:click="$toggle('showOrderNameModal')" wire:loading.attr="disabled">@lang('app.save')</x-button>
        </x-slot>
    </x-dialog-modal>
