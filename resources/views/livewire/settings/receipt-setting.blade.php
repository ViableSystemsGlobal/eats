        ...
        <form wire:submit="submitForm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                <label for="taxFormat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    @lang('modules.settings.taxFormat')
                                </label>
                                <div class="space-y-2">
                                    <div class="flex items-center">
                                        <input type="radio" name="taxFormat" id="taxFormatCurrent" value="current" 
                                               wire:model="taxFormat" class="mr-2">
                                        <label for="taxFormatCurrent" class="text-sm text-gray-700 dark:text-gray-300">
                                            <span class="font-medium">@lang('modules.settings.taxFormatCurrent')</span>
                                            <span class="text-xs text-gray-500 block">@lang('modules.settings.taxFormatCurrentDesc')</span>
                                        </label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="radio" name="taxFormat" id="taxFormatReverse" value="reverse" 
                                               wire:model="taxFormat" class="mr-2">
                                        <label for="taxFormatReverse" class="text-sm text-gray-700 dark:text-gray-300">
                                            <span class="font-medium">@lang('modules.settings.taxFormatReverse')</span>
                                            <span class="text-xs text-gray-500 block">@lang('modules.settings.taxFormatReverseDesc')</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                    ...
                    {{-- Footer Settings --}}
                    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700" x-data="{ footerImagePreview: @entangle('footerImage').defer }">
                        <div class="flex items-center justify-between mb-4">
                            <h4 class="text-base font-medium text-gray-900 dark:text-white">@lang('modules.settings.footerSettings')</h4>
                        </div>

                        <div class="space-y-4">
                            {{-- Footer Message --}}
                            <div>
                                <label for="footerMessage" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    @lang('modules.settings.footerMessage')
                                </label>
                                <textarea 
                                    id="footerMessage" 
                                    wire:model="footerMessage" 
                                    rows="3" 
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-skin-base dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    placeholder="@lang('modules.settings.footerMessagePlaceholder')"
                                ></textarea>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    @lang('modules.settings.footerMessageHelp')
                                </p>
                            </div>
                                ...
                                <div class="flex items-center space-x-6">
                                    ...
                                    <div class="flex-shrink-0">
                                        <div class="relative h-24 w-24">
                                            ...
                                            {{-- New Footer Image Preview --}}
                                            <div x-show="footerImagePreview" style="display: none;">
                                                <span class="block h-24 w-24 rounded-lg bg-cover bg-center bg-no-repeat"
                                                    x-bind:style="'background-image: url(' + footerImagePreview + ');'"></span>
                                            </div>

                                            {{-- Loading State --}}
                                            <div wire:loading wire:target="footerImage"
                                                class="absolute inset-0 bg-gray-900/60 rounded-lg flex items-center justify-center">
                                                <svg class="animate-spin h-6 w-6 text-white"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24">
                                    ...
                                    {{-- Upload Controls --}}
                                    <div class="flex-grow space-y-3">
                                        <input type="file" id="footerImage" class="hidden" accept="image/*" wire:model="footerImage" x-ref="footerImage"
                                            x-on:change="footerImageName = $refs.footerImage.files[0].name; const reader = new FileReader(); reader.onload = (e) => { footerImagePreview = e.target.result; };reader.readAsDataURL($refs.footerImage.files[0]);" />

                                        <div class="flex flex-wrap gap-3">
                                            <x-secondary-button type="button"
                                                x-on:click.prevent="$refs.footerImage.click()"
                                                class="flex items-center">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                ...
                                                @lang('modules.settings.uploadFooterImage')
                                            </x-secondary-button>
                                        </div>

                                        <x-input-error for="footerImage" class="mt-2" />
                                    </div>
                                </div>

                                {{-- Show/Hide Footer Image Option --}}
                                <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                    <x-checkbox name="showFooterImage" id="showFooterImage" wire:model='showFooterImage' />
                                    <label for="showFooterImage" class="ms-3 flex items-center">
                                        <svg class="w-5 h-5 text-gray-500 dark:text-gray-400 me-2" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 002 2z" />
                                        </svg>
                                        <span class="font-medium text-gray-900 dark:text-white">@lang('modules.settings.showFooterImage')</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="p-6">
                                            ...
                                            <div class="receipt" style="width: 80mm; margin: 0 auto;">
                                                ...
                                                <div class="summary">
                                                    ...
                                                    @if ($restaurantTax)
                                                        @if ($taxFormat === 'reverse')
                                                            {{-- Reverse Format Preview --}}
                                                            <div class="summary-row total">
                                                                <span>@lang('modules.order.total') (Base Food Price):</span>
                                                                <span>{{ currency_format(38.5) }}</span>
                                                            </div>
                                                            <div class="summary-row">
                                                                <span>@lang('modules.order.foodCost'):</span>
                                                                <span>{{ currency_format(32.6) }}</span>
                                                            </div>
                                                            <div class="summary-row">
                                                                <span>COVID (1%):</span>
                                                                <span>{{ currency_format(0.33) }}</span>
                                                            </div>
                                                            <div class="summary-row">
                                                                <span>VAT (15%):</span>
                                                                <span>{{ currency_format(5.57) }}</span>
                                                            </div>
                                                            <div class="summary-row total">
                                                ...
                                                <div class="footer">
                                                    ...
                                                    @if ($showFooterImage && $footerImage)
                                                        <div class="mt-2 text-center">
                                                            <img src="{{ $footerImage }}" alt="Footer Image" class="mx-auto" style="max-width: 200px; max-height: 100px;">
                                                        </div>
                                                    @endif
